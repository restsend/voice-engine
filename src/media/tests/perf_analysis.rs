...
mod tests {
    ...
    use crate::media::codecs::opus::OpusEncoder;
    use crate::media::codecs::pcmu::{PcmuDecoder, PcmuEncoder};
    use crate::media::codecs::resample::LinearResampler;
    use crate::media::codecs::{Decoder, Encoder};
    ...
    fn test_pipeline_performance() {
        ...
        // --- Common Components ---
        let mut resampler_8k_to_16k = LinearResampler::new(8000, 16000).unwrap();
        let noise_reducer = NoiseReducer::new(16000).unwrap();
        let vad_config = VADOption {
            samplerate: 16000,
            ..Default::default()
        };
        let mut vad = TinySilero::new(vad_config).unwrap();
        let mut resampler_16k_to_8k = LinearResampler::new(16000, 8000).unwrap();
